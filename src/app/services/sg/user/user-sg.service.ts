...
export class UserAPISG implements IUserAPI {
  ...
  public retrieveUser(user: User): Observable<User> {
    ...
    const query = `SELECT Id, Name, UserRole.Name,Company_OU__c,Team_Number_Mgr__c,FullPhotoUrl FROM User WHERE Id = '${user.userId}'`;
    ...
    return this.http.get<Query>(`${user.instanceUrl}/services/data/v48.0/query/?q=${urlFriendly}`, opt).pipe(
      map((res) => {
        if (res && res.records && res.records.length > 0) {
          ...
          user.userPhoto = res.records[0].FullPhotoUrl;
          user.teamName = res.records[0].Team_Number_Mgr__c;
          user.companyName = res.records[0].Company_OU__c;
